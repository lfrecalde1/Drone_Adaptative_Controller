...
set(gcf, 'PaperPositionMode', 'manual');
set(gcf, 'PaperPosition', [0 0 10 4]);
plot(t,params_estimados(:,1:length(t)),'linewidth',1); hold on;
plot(t,params_real(:,1:length(t)),'--','linewidth',1); hold on;
grid('minor')
grid on;
legend({'$\zeta$','$\zeta_r$'},'Interpreter','latex','FontSize',11,'Orientation','horizontal');
legend('boxoff')
%title('$\textrm{Evolution of Control Errors}$','Interpreter','latex','FontSize',9);
ylabel('$[m]$','Interpreter','latex','FontSize',9);
xlabel('$\textrm{Time}[s]$','Interpreter','latex','FontSize',9);
